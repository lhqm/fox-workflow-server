...
export function elMenuItem(h, menu) {
  if (menu.show) {
    ...
    else if (menu.iconSvg) icon = <icon-svg name={menu.iconSvg} />;
    else icon = <i class="fa fa-file-o" />;
    return (
      <el-menu-item key={menu.name || menu.path} index={menu.name}>
        {icon}
...
export function elSubmenu(h, menu) {
  if (menu.show) {
    ...
    else if (menu.iconSvg) icon = <icon-svg slot="title" name={menu.iconSvg} />;
    else icon = <i slot="title" class="fa fa-folder-o" />;
    return (
      <el-submenu key={menu.name || menu.path} index={menu.name}>
        {icon}
        <span slot="title">{menu.title || '未命名菜单'}</span>
