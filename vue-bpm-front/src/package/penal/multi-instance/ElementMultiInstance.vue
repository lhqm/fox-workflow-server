    ...
    <el-form label-width="90px" @submit.native.prevent>
      <el-form-item label="回路特性">
        <el-select v-model="loopCharacteristics" @change="changeLoopCharacteristicsType">
          ...
          <el-option label="顺序多重事件" value="SequentialMultiInstance" />
      ...
      <template v-if="loopCharacteristics === 'ParallelMultiInstance' || loopCharacteristics === 'SequentialMultiInstance'">
        ...
        <el-form-item label="集合" key="collection">
          <el-input readonly v-model="loopInstanceForm.collection" clearable />
        </el-form-item>
...
<!--        </el-form-item>-->
        <el-form-item  label="设置说明:" prop="overDueWarUrl">
          <span style="color: darkorange">设置或修改多人处理人规则后，需要保存发布才可生效</span>
        </el-form-item>
      </template>
    </el-form>
  </div>
...
export default {
  ...
  methods: {
    getElementLoop(businessObject) {
      businessObject=JSON.parse(JSON.stringify(this.muiElement.businessObject));
      ...
      this.multiLoopInstance = this.muiElement.businessObject.loopCharacteristics;
      // 更新表单
      if (
    ...
    changeLoopCharacteristicsType(type) {
      ...
      // 取消多实例配置
      if (type === "Null") {
        window.bpmnInstances.modeling.updateProperties(this.muiElement, { loopCharacteristics: null });
        return;
      }
      // 配置循环
      if (type === "StandardLoop") {
        const loopCharacteristicsObject = window.bpmnInstances.moddle.create("bpmn:StandardLoopCharacteristics");
        window.bpmnInstances.modeling.updateProperties(this.muiElement, {
      ...
      const multiBehavior=this.muiElement.businessObject.candidateUsers;
      if(multiBehavior.length<=0||multiBehavior==""||multiBehavior==null){
        return;
      }
      else{
        let multiBehaviorRep=multiBehavior.replace("}","");
        multiBehaviorRep=multiBehaviorRep.replace("{","");
        multiBehaviorRep=multiBehaviorRep.replace("$","");
        multiBehaviorRep=multiBehaviorRep+"List";
        this.loopInstanceForm.collection=multiBehaviorRep;
        window.bpmnInstances.modeling.updateModdleProperties(this.muiElement, this.multiLoopInstance, {
    ...
    updateLoopAsync(key) {
      ...
      window.bpmnInstances.modeling.updateModdleProperties(this.muiElement, this.multiLoopInstance, asyncAttr);
    }
  },
  beforeDestroy() {
    this.multiLoopInstance = null;
