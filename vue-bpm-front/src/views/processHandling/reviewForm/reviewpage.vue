    ...
    <el-tab-pane label="流程审核" style="height: 100%; padding-bottom: 20px">
      <el-container v-loading="loading">
        <el-header style="text-align: right; height: auto">
          <el-card>
            <el-button icon="el-icon-remove" type="danger" v-if="toolBar.end" @click="actionClick('setEndTask')">结束</el-button>
            <el-button v-if="toolBar.ccWay=='manualCC'" icon="el-icon-position" @click="actionClick('copyFor')">知会</el-button>
            <el-button icon="el-icon-s-custom" v-if="toolBar.transfer" @click="actionClick('transfer')">移交</el-button>
            <el-button icon="el-icon-share" v-if="toolBar.countersign" @click="actionClick('countersign')">加签</el-button
            >
            <el-button icon="el-icon-s-release"  v-if="toolBar.refuse" @click="actionClick('refuse')">不同意</el-button>
            <el-button  v-if="toolBar.returnWork!='none'" @click="showReturnModal"
              >驳回</el-button
            >
            <el-button icon="el-icon-s-claim"  @click="save">保存</el-button>
            <el-button icon="el-icon-success" type="primary" v-if="toolBar.processStart" @click="processStart"
              >提交</el-button
            >
            <el-button icon="el-icon-success" type="primary" v-if="toolBar.sendWork" @click="sendWork">同意</el-button>
        ...
        <el-main style="flex-grow: 1; height: 80vh">
          <div style="overflow-y: auto; height: 100%">
            <slot>
              <preview v-loading="loading"
                       element-loading-text="请等待..."
                       element-loading-spinner="el-icon-loading" v-if="form_type=='0'"
                :itemList="itemList"
                :itemDataJson="itemDataJson"
                :formConf="formConf"
                ref="preview"
              />
              <iframe ref="previewIframe" v-if="form_type=='1'" :src="form_url" style="width: 100%;height: 100%" frameborder="0"></iframe>
            </slot>
            <slot
              style="height: 100%"
              v-if="this.procinstid != '' && this.procinstid != null && this.procinstid != undefined"
            >
            ...
            <slot name="-" style="border: none; padding: 0px">
              ...
              >
                ...
                <el-form-item style="margin-right: 100px">
                  ...
                  <el-button @click="dialogNewSortVisible=false;loading=false;">取消</el-button>
                </el-form-item>
              </el-form>
            </slot>
          </el-dialog>
          ...
          >
            <slot name="-" style="border: none; padding: 0px">
              <el-form
                ...
                class="demo-ruleForm"
              >
                <el-form-item :label="selectLabel" v-if="actionType=='transfer' || actionType=='countersign'" prop="selectEmp">
                ...
                <el-form-item style="margin-right: 100px">
                  <el-button v-if="actionType=='transfer'" type="primary" @click="transferClick">确认</el-button>
                  <el-button v-if="actionType=='countersign'" type="primary" @click="countersignClick">确认</el-button>
                  <el-button v-if="actionType=='refuse'" type="primary" @click="refuseClick">确认</el-button>
                  <el-button v-if="actionType=='setEndTask'" type="primary" @click="setEndTaskClick">确认</el-button>
                  <el-button @click="dialogSelectVisible=false;loading=false;">取消</el-button>
                </el-form-item>
              </el-form>
            </slot>
          </el-dialog>
          ...
          >
            <slot name="-" style="border: none; padding: 0px">
              <el-form
                ...
                class="dialog-form"
              >
                <el-form-item v-if="approval.variables.expType==='ExclusiveGateway'"
                              v-for="item in nextNodes" :key="item.id">
                  <el-form-item>
                    <el-radio v-model="approval.variables.nextNode" :label="item.id">{{item.name}}</el-radio>
                  </el-form-item>
                </el-form-item>
                <el-form-item v-if="approval.variables.expType==='InclusiveGateway'">
                  <el-checkbox-group v-model="checkedNodes" >
                    <el-checkbox v-for="node in nextNodes" :label="node.id" :key="node.id">{{node.name}}</el-checkbox>
                  </el-checkbox-group>
                </el-form-item>
              </el-form>
          ...
          >
            ...
            >
              <el-form-item v-if="resType=='suecess'">
                <p><i class="el-icon-check" style="color: #55a532;font-size: x-large;font-weight: 800;"></i>&nbsp;&nbsp;操作成功。</p>
                <p><i class="el-icon-info" style="color: #55a532;font-size: x-large;font-weight: 800;"></i>&nbsp;&nbsp;{{this.resTypeMsg}}</p>
                <p><i class="el-icon-video-camera-solid" style="color: #55a532;font-size: x-large;font-weight: 800;"></i>&nbsp;&nbsp;您可以在已办任务中，查看流程审核轨迹。</p>
              </el-form-item>
              <el-form-item v-if="resType=='error'">
                <p><i class="el-icon-warning" style="color: #bd2c00;font-size: x-large;font-weight: 800;"></i>&nbsp;&nbsp;操作失败。</p>
                <p><i class="el-icon-warning" style="color: #bd2c00;font-size: x-large;font-weight: 800;"></i>&nbsp;&nbsp;{{this.resTypeMsg}}</p>
                <p><i class="el-icon-message-solid" style="color: #0d72ff;font-size: x-large;font-weight: 800;"></i>&nbsp;&nbsp;您可联系管理员检查流程配置是否正确。</p>
              </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
              <el-button v-if="resType=='error'" @click="dialogVisible = false"> 确定 </el-button>
              <el-button v-if="resType=='suecess'" type="primary" @click="cloaseDialog"> 确定 </el-button>
            </div>
          </el-dialog>
        </el-main>
...
<script>
  ...
  import { startProcess, sendWork, returnWork, transfer
    , refuse, countersign } from '@/api/task/task';
  ...
  export default {
    ...
    methods: {
      ...
      save() {
        ...
        let mapString = JSON.stringify(mapConfig);
        let generid = '';
        ...
        saveFormDataJson(this.processKey,generid,this.form_type,this.form_url+"?businessKey="+generid, mapString, JSON.stringify(refList.keyValueList)).then((res) => {
          if (res.error == '200') {
      ...
      processStart() {
        ...
        let newRefList = refList.keyList.filter((item) => {
          if (
            item.compType == 'text' ||
            ...
            startProcess(this.processKey, this.processName, this.bussid).then((res) => {
              if (res.error == '200') {
                ...
                if(res.error=='500')
                  ...
                  this.resTypeMsg=res.result.msg;
                this.loading=false;
                this.dialogVisible=true;
              }
            });
      ...
      sendWork() {
        ...
        saveFormDataJson(this.processKey,generid,this.form_type,this.form_url, mapString, JSON.stringify(refList.keyValueList)).then((res) => {
          if (res.error == '200') {
